...
The `dbt/warehouse` project (`warehouse_transform`, PostgreSQL) owns the dbt
staging models and the four `marts.v_*` views, and is tested at the levels below,
each answering a different question. This is the *what and how to run it*; the rationale for each layer — and for what is deliberately **not**
tested — lives in
...
| Architecture gate | Do models respect the source → staging → marts boundary? | `dbt/warehouse/.dbt-doctor`, see [W4](warehouse/W4-dbt-architecture-gate.md) |
...
Architecture gate (**no database needed**; `dbt parse` does not connect). The
manifest must be regenerated first — a stale one gates the wrong graph:
...
Expect exit 0, zero errors and two `source-childs` warnings; both warnings are
explained in [W4](warehouse/W4-dbt-architecture-gate.md#the-two-warnings-that-stay).
Do not add `--score` — on the pinned 0.3.4 it suppresses the failing exit code.
