...
Orders streaming (`spark/jobs/orders_streaming.py`): `KAFKA_BOOTSTRAP_SERVERS` (`kafka:9092`), `KAFKA_TOPIC` (`orders`), `MINIO_BUCKET` (`de-practicum`), `RAW_OUTPUT_PATH` (`s3a://de-practicum/streaming/orders_raw`), `RAW_CHECKPOINT_PATH`, `POSTGRES_CHECKPOINT_PATH`, `POSTGRES_HOST`/`PORT`/`DB`/`USER`/`PASSWORD` (defaults `de-demo-postgres`/`5432`/`dwh`/`app`/`app`).
