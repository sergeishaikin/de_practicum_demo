...
2. The single-active-run DAG truncates and loads the `stg` staging tables, then blocks core/marts mutation until all four non-empty CSV inputs exactly match their staging row counts. It rebuilds the `core` tables from SQL in `db/pipeline_sql/`, runs a payment reconciliation quality gate, and writes an audit row to `marts.pipeline_runs`.
...
- **Airflow TaskFlow DAG with `chain()`** — a linear DAG of `load_raw_csv_to_stg`, `validate_staging`, `rebuild_core_and_marts`, `check_payment_reconcile`, and `write_audit` tasks. The staging gate has four staging inlets and no outlet. See `dags/demo_core_marts_pipeline.py`.
