...
graph LR
  ...
  ING --> STG[(PostgreSQL stg)]
  STG --> CORE[(PostgreSQL core)]
  CORE --> ASSET[core.orders AssetEvent<br/>row counts + source DagRun]
  ASSET --> VALIDATE[Airflow<br/>warehouse_marts_validation]
  VALIDATE --> MARTS[(PostgreSQL marts views<br/>pipeline_runs audit)]
