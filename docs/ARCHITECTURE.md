...
`warehouse_transform` project, and dbt owns two layers rather than one. Four thin
staging models — `staging.stg_core__*` and `staging.stg_staging__*` — are the
only place a raw `core.*` or `stg.*` relation is read; the four `marts.v_*` views
are built from them through `ref()`. Two layers here are called "staging" and are
not the same thing: `stg.*` is Airflow's CSV arrival point, `staging.*` is dbt's
normalization boundary. The boundary is enforced on every pull request — see
