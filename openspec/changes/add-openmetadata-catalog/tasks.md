...
- [ ] Resolve the current stable OpenMetadata server and ingestion release and
  record immutable registry digests; distinguish stable from pre-release.
- [ ] Resolve OpenSearch and Postgres compatibility and record the exact image
  identities selected for the isolated profile.
- [ ] Capture primary-source requirements for Airflow, dbt, Kafka, Trino,
  OpenLineage, Iceberg, authentication, and resource sizing.
- [ ] Compare repository versions and configuration with those requirements,
  including the actual generated dbt artifacts and compiled SQL.

## Wave B — isolated control-plane preflight

- [ ] Start only a temporary, opt-in metadata profile with fresh declared
  volumes/networks and a dedicated metadata fixture namespace.
- [ ] Prove metadata DB schema/user isolation, OpenSearch health/index/search,
  server/ingestion health, restart, and clean shutdown.
- [ ] Measure startup, steady-state, peak memory/CPU, disk, and host headroom;
  keep vendor, repo allocation, and measured values separate.

## Wave C — connector and lineage probes

- [ ] Probe Airflow metadata through the existing deployment using read-only
  access; do not add a second Airflow.
- [ ] Ingest the real dbt manifest/catalog/run-results artifacts and verify a
  representative model plus a required column-lineage subset.
- [ ] Probe isolated Kafka topic metadata and a fixture event without changing
  business topics, offsets, or stale checkpoints.
- [ ] Probe Trino/Iceberg discovery and verify canonical FQN aliasing.
- [ ] Deliver one isolated OpenLineage event through Kafka using existing
  emitter semantics and verify runtime-edge ownership, run IDs, and no duplicate
  edge authority.

## Wave D — guardrails and decision

- [ ] Verify secret redaction/read-only permissions and catalog-outage fail-open
  behaviour with an observable counter/log.
- [ ] Destroy only the preflight profile and rebuild from clean state; compare
  identity, search, lineage, and artifact results.
- [ ] Record every unsupported connector, unresolved dataset, and the existing
  NG-0.2 streaming gap as explicit limitations rather than inferred edges.
- [ ] Produce the full OM-PREFLIGHT report with evidence matrices and exactly
