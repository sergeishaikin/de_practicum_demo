...
- [ ] 3.1 Push and run M5 with the instrumentation attached
- [ ] 3.2 If it reproduces, download the captured bytes and classify against A/B/C/D/E
- [ ] 3.3 If it does not reproduce, run M5 again on the same head and record repeatability across isolated runs
- [ ] 3.4 Record the classification with the bytes that support it, or `classification not established`

## 4. Fix, if the evidence names one

- [ ] 4.1 Choose the fix from the established class, not from the most plausible story
- [ ] 4.2 Add a regression test demonstrated to fail without the fix
- [ ] 4.3 Verify: focused reproducer, M3/M4 integration gates, BDD M5 gates, full suite, live M5

## 5. Closure

- [ ] 5.1 Write `evidence.md` with the classification, the bytes that support it, and any deviation
