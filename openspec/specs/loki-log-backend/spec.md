...
- **WHEN** an operator opens a Tempo span for an adopted first-party service
- **THEN** Grafana can query Loki for a log carrying the same trace ID
...
### Scenario: Forbidden value reaches an error path

- **WHEN** a representative secret, credential, sensitive SQL literal, PII, or
  full payload reaches an in-scope logging path
- **THEN** a query against persisted Loki content finds none of those values
- **AND** useful event and execution context remains.
