...
The standing specification for the adopted NG-0.4 observability boundary.
...
NOT be Prometheus time-series labels or dimensions; Kafka payloads, secrets
and arbitrary PII SHALL NOT be recorded by default. `trace_id` and `span_id`
MAY be attached as bounded exemplar metadata to an existing application metric
observation for correlation, but exemplar metadata SHALL NOT become business
or SLO grouping authority, increase series cardinality, or justify
span-derived metrics.

#### Scenario: Kafka context is asynchronous

- **WHEN** a consumer processes a message whose W3C context was injected by
  the producer
...
Network exporters SHALL declare finite queue capacity and retry horizon.
Signal classes that require restart recovery SHALL use persistent
`file_storage` WAL with an explicit disk budget. Queue-full, retry-timeout,
exporter failure, receiver error and refused/dropped telemetry SHALL be
exposed through Prometheus-scrapable Collector/application metrics.
...
be explicitly configured and SHALL preserve errors and critical-path
diagnostics when sampling is introduced.
