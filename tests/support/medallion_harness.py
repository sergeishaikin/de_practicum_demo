...
canonical ``marts`` schema is never touched).
"""
...
def wait_for_new_gold_snapshot(
    cat: RestCatalog, namespace: str, previous: int, timeout: int = 90
) -> None:
    ...
    deadline = time.time() + timeout
    while time.time() < deadline:
        if gold_snapshot_count(cat, namespace) > previous:
            return
        time.sleep(1)
    raise AssertionError(
        f"{namespace}.gold gained no snapshot within {timeout}s — "
        "the deployment never completed a cycle"
    )
...
def run_deployment(
    ...
    *,
    ...
    Every deployment waits for a *new* Gold snapshot, which is the proof that
    this deployment ran rather than inheriting the previous stage's output.
    """

    cat = catalog()
    gold_before = gold_snapshot_count(cat, namespace)
    ...
    try:
        ...
        wait_for_new_gold_snapshot(cat, namespace, gold_before)
